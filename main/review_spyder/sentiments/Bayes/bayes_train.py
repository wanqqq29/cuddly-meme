...
Hanlp = hanlp.pipeline().append(hanlp.utils.rules.split_sentence).append(tok).append(
    ...
    # 通过print工作路径可知，运行在"D:\0x1毕设\main"中，所以要以项目根路径开始
    lambda sents: remove_stop_words(sents, 'review_spyder/sentiments/stop.txt'))
...
def pos_stl(array):
    ...
    for i in range(0, len(pos_list)):
        # 排除名词、地名
        if pos_list[i] != 'NN' and pos_list[i] !='NR':
        # if pos_list[i] == 'VA':
            list.append(array[i])
    return list
...
def 模型训练():
    ...
    yy_pre = clf.predict_proba(x_test)  # adaboost用到的概率
